...
redis.select(process.env.REDIS_DB || config.get('redis_db'));
...
function setRights(req, res, next) {
    ...
    secure.fetch(function(err) {
        ...
        var hashedToken = encryptPassword(config.get('planner_password'), userId);
...
expressApp
  ...
  .use(express.static(__dirname + '/../../public', { maxAge: config.get('static:maxAge') }))
    ...
    db.collection('courses').find({ name: { $ne: null } })
        .toArray(function (err, items) {
            ...
            items.forEach(function (item) {
                var extractedItem = { name: item.name, price: item.price, type: item.type };
                if (item.isSalad) extractedItem.isSalad = true;
                dump.push(extractedItem);
            });
...
expressApp.post('/api/loadCourses', function(req, res, next) {
    if (req.files.courses.size > config.get('maxUploadSize')) return next();
