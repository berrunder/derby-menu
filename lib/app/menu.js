...
app.on('model', function(model) {
    ...
    model.fn('updateOrder', function(user, courses) {
        ...
        for (var id in userCourses) if (userCourses.hasOwnProperty(id)) {
            if (userCourses[id]) {
                ...
                order.coursesSum += normalizeUInt(course.price);
            }
        }
        ...
        if (breadSum > 0) {
            var breads = [];
            var white = normalizeUInt(user.bread.white),
                black = normalizeUInt(user.bread.black);
    ...
    var normalizeUInt = function(value) {
