...
app.view.fn('getOrderText', function(user, courses) {
    ...
    for (var id in uCourses) if (uCourses.hasOwnProperty(id)) {
        ...
        if (!course || course.date != user.lastDate) continue;
    ...
    sCourseList = helpers.lowerNotFirst(sCourseList);

    if (sCourseList.length > 0) {
    	var sBread = helpers.countBread(user);
        if (sBread.length > 0) sCourseList += ', ' + sBread;
    }
