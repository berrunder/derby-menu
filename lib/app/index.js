...
app.on('model', function (model) {
    ...
    model.fn('filterFirstCourses', function(item, key, object) {
        if (!item.name || item.type != 1) return false;
    ...
    model.fn('filterSecondCourses', function(item, key, object) {
        if (!item.name || item.type != 2) return false;
...
app.fn('submitToMenu', function(e, el) {
    ...
    if (firstItem.date === sNow) {
        this.model.del('courses.' + firstItem.id + '.date');
    } else {
        this.model.set('courses.' + firstItem.id + '.date', sNow);
    }
});
